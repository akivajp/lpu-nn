...
- 同じコードベースの残り 3 つ。文字言語モデル (`LanguageModel`,
  `lpu-nn-train-embedding`)、トークナイザコマンド
  (`lpu-nn-run-tokenizer`)、学習済み系列変換モデルの HTTP サーバ
  (`lpu-nn-serve-seq2seq`)。これで元コードは全て移植済みとなる。
- `serve` エクストラ。サーバが必要とする唯一の依存をここに置く。
  既定のインストール内容は変わらない。
- 言語モデルを 1 エポック訓練し、それが学習した語彙にトークナイザを
  適用する CI スモークテスト。
...
`modeling/language_models.py` と `train_embedding.py` が、`modeling` の
旧称である `modules` を基底クラスに使っており、どちらも import できな
かった。

`serve_seq2seq` は読み込み時に環境変数を 2 つ設定しており、モジュールを
読み込んだだけでプロセス全体のデバッグ出力が有効になっていた。変数名も
本パッケージの改名前の名称のままだった。

サーバは、実行環境の地方時に日本時間の札を貼っていた (その時間帯で時刻を
取っていなかった) ため、日本時間以外の環境では誤った時刻を報告していた。
標準ライブラリの `zoneinfo` を使うようにし、`pytz` は不要になった。

サーバは全インターフェースで待ち受け、bottle のデバッグモードと自動
再起動を強制していた。`--host` を指定しない限り `127.0.0.1` のみで待ち
受け、例外の内容を要求元へ返さず、再起動も指定時のみとした。

`LanguageModel.prepare_batch` は、文字列 1 件のときだけ `pad_sequence` へ
2 次元テンソルを渡しており (他の枝はテンソルの列を渡す)、未対応の型では
未代入の変数を参照していた。

`EmbeddingTrainer.feed_one_batch` は、基底トレーナーが `df` を渡すように
なる前の署名のままだった。生じる `TypeError` は訓練ループに捕まるため、
実行は成功と報告されながら何も学習していなかった。

`run_tokenizer` はロギング設定の対象をパッケージ名ではなくロガー
オブジェクトにしていたため、`--debug` が自身の出力にしか効かなかった。
引数名 `format` が組み込み関数を隠してもいた。

`LanguageModel.reset_state` が復号器の状態更新を
`hasattr(...) and ...` という、値を捨てる式で書いていた。
